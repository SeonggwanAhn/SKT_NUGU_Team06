...
def test_target(args, is_fine_tuned=True):
    dset_loaders = digit_load(args)
    ## set base network
    if args.dset == 'u2m':
        netF = network.LeNetBase().cuda()
        ...
        netB = network.feat_linear(type=args.classifier, input_dim= netF.in_features, output_dim=args.bottleneck).cuda()
    elif args.dset == 'm2u':
        netF = network.LeNetBase().cuda()
        netB = network.feat_bootleneck(type=args.classifier, feature_dim=netF.in_features, bottleneck_dim=args.bottleneck).cuda()
    elif args.dset == 's2m':
    ...
    if is_fine_tuned:
        args.modelpath = args.pretrained_dir + '/target_F_'+args.savename+'.pt'
        netF.load_state_dict(torch.load(args.modelpath))
        args.modelpath = args.pretrained_dir + '/target_B_'+args.savename+'.pt'
        netB.load_state_dict(torch.load(args.modelpath))
        args.modelpath = args.pretrained_dir + '/target_C_'+args.savename+'.pt'
        netC.load_state_dict(torch.load(args.modelpath))
    else:
        args.modelpath = args.pretrained_dir + '/source_F.pt'
        netF.load_state_dict(torch.load(args.modelpath))
        args.modelpath = args.pretrained_dir + '/source_B.pt'
        netB.load_state_dict(torch.load(args.modelpath))
        args.modelpath = args.pretrained_dir + '/source_C.pt'
        netC.load_state_dict(torch.load(args.modelpath))
    netF.eval()
    netB.eval()
    netC.eval()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='s2m', choices=['u2m', 'm2u','s2m', 'm2u_half', 'm2m'])
    ...
    args.pretrained_dir = 'ckps_digits/seed2020/'+args.pretrained_dir+'/split_model/student'
    if not osp.exists(osp.join(args.pretrained_dir + '/source_F.pt')):
