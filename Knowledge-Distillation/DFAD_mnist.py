...
import random
import numpy as np
from network import newlenet
from network import Nlenet
import torchvision
...
def main():
    ...
    teacher = network.Nlenet.LeNet5()

#    nF = network.Tlenet.LeNetBase().cuda()
#    nB = network.Tlenet.feat_bootleneck(type='bn', feature_dim=nF.in_features, bottleneck_dim=256).cuda()
#    nC = network.Tlenet.feat_classifier(type='wn', class_num=10, bottleneck_dim=256).cuda()

#    teacher = network.Tlenet.MergeModels(nF, nB, nC)


    student = network.Nlenet.LeNet5Half()
#    netF = network.newlenet.LeNetBase().cuda()
#    netB = network.newlenet.feat_bootleneck(type='bn', feature_dim=netF.in_features, bottleneck_dim=256).cuda()
#    netC = network.newlenet.feat_classifier(type='wn', class_num=10, bottleneck_dim=256).cuda()
    
#    student = network.newlenet.MergeModels(netF, netB, netC)



    #student = network.lenet.LeNetBase()
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if acc>best_acc:
            best_acc = acc
            torch.save(student.state_dict(),"checkpoint/student/%s-%s-do.pt"%('mnist', 'lenet5_half'), _use_new_zipfile_serialization=False)
